...
import com.riozenc.expression.Evaluable;
import com.riozenc.expression.Evaluator;
		...
		if (evaluator != null && evaluator.canOperator(op, firstObj, secondObj)) {
			...
			if (m != null) {
				...
				throw new IllegalExpressionException(
						"不支持:" + op.getToken() + "操作, 在对象:" + firstObj.getClass().getName());
			}
		}
	...
	private Method getOperator(Operator op, Object obj) {
		...
		if (!evalMethod.containsKey(ck)) {
			...
			for (Method om : obj.getClass().getMethods()) {
				com.riozenc.expression.annotation.Operator anno = om
						.getAnnotation(com.riozenc.expression.annotation.Operator.class);
